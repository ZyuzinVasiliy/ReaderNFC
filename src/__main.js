...
/*
                                window.nfc.addNdefListener( // addNdefFormatableListener
                                    (nfcEvent) => {
                                        alert('Карта прочитана');
                                    ...
                                    () => {
                                        alert('Ошибка оборудования')
                                    },
                                )
*/
                                alert('Ридер готов, поднесите карту')
                                window.nfc.addTagDiscoveredListener(handleDesfire); // addTagDiscoveredListener  addNdefFormatableListener
...
async function handleDesfire(nfcEvent) {
    ...
    try {
        alert('Подключаемся к тегу с ID ' + tagId);
        await window.nfc.connect('android.nfc.tech.MifareClassic', 100);
        alert('Успешно подключено');

        let data = window.nfc.transceive([0x40]);
        alert(data);
        alert('Result: ' + JSON.stringify(util.arrayBufferToHexString(data)));
    ...
    transceive: function(data) {
        return new Promise(function(resolve, reject) {
            ...
            if (typeof data === 'string') {
                ...
                alert("Expecting an ArrayBuffer or String");
